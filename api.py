...
class Vocab:
    ...
    def __init__(self, VOCAB_PATH):
        ...
        with open(VOCAB_PATH, 'rb')as f:
            ...
            self.vocab = my_vocab
            self.ix2word = my_vocab.get_itos()
            self.word2ix = my_vocab.get_stoi()
...
def sample_from_model(model,
    ...
    samples, z = model.generate_sentences(
...
def recon_sequence(model,
    ...
    samples = sample_from_model(
        model, vocab, z,  n_samples, **mb_sample_kwargs)
    return samples
...
def get_model_and_vocab_path():
    ...
    MODEL_PATH = '{}/model_{}.pt'.format(base, cfg.wae.n_iter)
