...
def gaussian_sample(z):
    """ sample according to N(0,1) """
...
def recon_dec(sequences, logits):
    ...
    # dec_inputs: '<start> I want to fly <eos><pad>'
    # dec_targets: 'I want to fly <eos> <pad><pad>'
    ...
    recon_loss = F.cross_entropy(  # this is log_softmax + nll
        logits.view(-1, logits.size(2)), dec_targets.view(-1),
        ignore_index=PAD_IDX  # padding doesnt contribute to recon loss & gradient
    )
    return recon_loss


def loss_classifier(preds,labels):
    preds = torch.clamp(preds,min=1e-7,max=1-1e-7)
    loss = -(torch.log(preds)*labels+torch.log(1-preds)*(1-labels))
